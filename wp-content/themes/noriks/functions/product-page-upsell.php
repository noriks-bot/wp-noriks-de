 ...
 *
 * Okvir se prikazuje ODMAH ISPOD gumba "Dodaj u košaricu" i nudi 4x Grüne Boxershorts
	...
	acf_add_local_field_group( array(
		...
		'fields' => array(
			array(
				...
				'label'        => 'Upsell unter dem Button anzeigen (4x Grüne Boxershorts)',
			...
			array(
				...
				'label'        => 'Upsell #3 unter dem Button anzeigen (NORIKS FIT Kompressionsshirt)',
				'name'         => 'noriks_pp_upsell3',
				'type'         => 'true_false',
				'instructions' => 'Dritte Upsell-Box: NORIKS FIT Kompressionsshirt, der Kunde wählt die Größe. Unabhängig von den ersten beiden Schaltern.',
				'ui'           => 1,
			),
		),
		'location'   => array(
...
function noriks_pp_upsell_render() {
	...
	?>
	<div class="npu-wrap">
		<span class="npu-label">Zusammen kaufen und sparen:</span>
			...
			<div class="npu-grid">
				...
				<div class="npu-actions">
					<label class="npu-check">
						...
						<span class="npu-check-text">Zum Kauf hinzufügen</span>
					</label>

					<select class="npu-size" name="noriks_pp_upsell_size" aria-label="Größe der Boxershorts">
...
function noriks_pp_upsell2_config() {
	return apply_filters( 'noriks_pp_upsell2_config', array(
		...
		'label'      => 'Zusammen kaufen und sparen:',
		'add_text'   => 'Zum Kauf hinzufügen',
		'aria'       => 'Größe der T-Shirts',
	) );
}
...
function noriks_pp_upsell2_maybe_add( $cart_item_key, $product_id, $quantity, $variation_id, $variation, $cart_item_data ) {
	...
	$lines  = array(
		( $prod_a ? $prod_a->get_name() : 'Schwarzes' ) . ' - ' . $size,
		( $prod_b ? $prod_b->get_name() : 'Graues T-Shirt' ) . ' - ' . $size,
	);
...
function noriks_pp_upsell3_config() {
	return apply_filters( 'noriks_pp_upsell3_config', array(
		'product_id' => 7791,   // NORIKS FIT kompresijska majica (jednostavan proizvod)
		'total'      => 17.99,  // cijena za 1 komad
		'title'      => 'NORIKS FIT Kompressionsshirt',
		'desc'       => 'Formt die Silhouette und stützt die Haltung — füge es mit %s%% Rabatt hinzu.',
		'size_attr'  => 'Größe',
...
function noriks_pp_upsell3_order_item_meta( $item, $cart_item_key, $values, $order ) {
	if ( ! empty( $values['_noriks_pp_upsell3'] ) ) {
		...
		if ( ! empty( $values['_noriks_pp_upsell_size'] ) ) {
			$item->update_meta_data( 'Größe', sanitize_text_field( $values['_noriks_pp_upsell_size'] ) );
		}
	}
}
