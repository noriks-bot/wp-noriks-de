...
function noriks_ajax_add_to_cart() {
    ...
    if ($cart_item_key) {
        ...
        $msg = !empty($notices) ? strip_tags($notices[0]['notice'] ?? $notices[0]) : 'Fehler beim Hinzufuegen';
        wc_clear_notices();
...
function noriks_upsell_modal_markup() {
        ...
        <div class="noriks-modal">
            ...
            <div class="noriks-modal-body">
                ...
                <div class="noriks-modal-qty-row">
                    <span class="noriks-attr-label">MENGE</span>
                ...
                <div id="noriks-modal-error" class="noriks-modal-error" style="display:none;">Waehlen Sie alle Optionen</div>
                <button id="noriks-modal-add" class="noriks-modal-add-btn">IN DEN WARENKORB</button>
            </div>
    ...
    (function($) {
        ...
        function renderModal() {
            ...
            $('#noriks-modal-add').text('IN DEN WARENKORB').removeClass('adding added');
        ...
        $(document).on('click', '#noriks-modal-add', function() {
            ...
            $.post(woocommerce_params.ajax_url, data, function(res) {
                ...
                if (res.success !== false && res.fragments) {
                    ...
                    $btn.removeClass('adding').text('IN DEN WARENKORB');
                    var msg = (res.data && res.data.message) ? res.data.message : 'Fehler beim Hinzufuegen';
                ...
                $btn.removeClass('adding').text('IN DEN WARENKORB');
                $('#noriks-modal-error').text('Fehler beim Hinzufuegen').show();
            });
        });
