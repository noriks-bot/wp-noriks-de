 ...
 *
 * Step 1: Single product offer (boxershorts)
 * Step 2: 6-product grid (after "Nein danke" or after adding 1 item)
 *
...
// Check if order has ONLY boxershorts (no t-shirts, no komplet)
$has_only_boxershorts = true;
$has_boxershorts = false;
...
if ( $upsell_is_t-shirts ) {
    ...
    $upsell_qty_names  = array( 1 => '1x Schwarzes T-Shirt', 3 => '3x Schwarze T-Shirts', 6 => '6x Schwarze T-Shirts' );
    $upsell_qty_images = array(
        1 => 'https://noriks.com/de/wp-content/uploads/2025/09/black-1.jpg',
        3 => 'https://noriks.com/de/wp-content/uploads/2025/09/black-3x.jpg',
        6 => 'https://noriks.com/de/wp-content/uploads/2026/01/15xcrnat-shirt.png',
    );
        ...
        if ( is_a( $item, 'WC_Order_Item_Product' ) && $item->get_variation_id() ) {
            ...
            if ( $var ) {
                foreach ( $var->get_attributes() as $k => $v ) {
                    if ( stripos( $k, 'groesse' ) !== false || stripos( $k, 'size' ) !== false ) {
...
// Try boxershorts/t-shirts categories first
$grid_products = array();
foreach ( array( 'boxershorts', 'boxerice', 't-shirts', 't-shirt' ) as $cat_slug ) {
...
<div id="order-splash" style="position:fixed;top:0;left:0;width:100%;height:100%;background:#04ac00;z-index:999999;display:flex;flex-direction:column;align-items:center;justify-content:center;transition:opacity 0.6s ease;">
    ...
    <h1 style="color:#fff;font-family:'Roboto',sans-serif;font-size:28px;font-weight:700;margin:0;">Bestellung eingegangen!</h1>
    <p style="color:rgba(255,255,255,0.85);font-family:'Roboto',sans-serif;font-size:15px;margin:10px 0 0;">Bestellnummer: #<?php echo $order->get_order_number(); ?></p>
</div>
...
<div class="ty-container">
    ...
    <?php if ( $order->has_status( 'failed' ) ) : ?>
        <div class="ty-success" style="background:#fde8e8;">
            ...
            <p>Die Bank hat die Transaktion abgelehnt. Bitte versuchen Sie es erneut.</p>
            <p style="margin-top:16px;">
                <a href="<?php echo esc_url( $order->get_checkout_payment_url() ); ?>" style="display:inline-block;background:#E8450E;color:#fff;padding:12px 32px;text-decoration:none;font-weight:700;">Erneut versuchen</a>
            </p>
        </div>
    <?php else : ?>
        ...
        <div class="ty-success">
            ...
            <p>Die Bestaetigung wurde gesendet an <?php echo esc_html( $order->get_billing_email() ); ?></p>
            <span class="ty-order-num">Bestellung #<?php echo $order->get_order_number(); ?></span>
        </div>
            ...
            <div class="ty_upsell_one_wrapper__popup-content">
                ...
                <div class="tyuo_timer" style="position:relative;">
                    <div class="timer_wrapper">
                        <div class="special_offer_txt">Letzte Chance - Angebot endet in</div>
                ...
                <div class="tyuo_middle_section">
                    <div class="sub_title">
                        ...
                        Ohne zusaetzliche Versandkosten - alles in einem Paket
                    </div>
                    <div class="clue_text">
                ...
                <div class="tyuo_product_section">
                    ...
                    <div class="ty-qty-picker" style="display:flex;gap:8px;padding:0 0 10px;justify-content:center;">
                        ...
                        <label class="ty-qty-btn<?php echo $cls; ?>" style="flex:1;text-align:center;padding:10px 0;border:2px solid <?php echo $border; ?>;border-radius:4px;font-weight:700;font-size:14px;cursor:pointer;background:<?php echo $bg; ?>;color:#000;">
                            <input type="radio" name="ty_qty" value="<?php echo $q; ?>"<?php echo $chk; ?> style="display:none;"> <?php echo $q; ?>x Stk.
                        </label>
                        <?php endforeach; ?>
                    </div>
                        ...
                        <select class="variation-select" id="ty-variation-select">
                            <?php if ( $upsell_variations ) : ?>
                                ...
                                <option value="<?php echo $v['id']; ?>" <?php selected( strtolower($v['size']), strtolower($customer_size) ); ?>>
                                    Schwarz, <?php echo esc_html( $v['size'] ); ?>
                                </option>
                                <?php endforeach; ?>
                            <?php else : ?>
                                <option value="">Schwarz, S</option>
                                <option value="">Schwarz, M</option>
                                <option value="">Schwarz, L</option>
                                <option value="">Schwarz, XL</option>
                                <option value="">Schwarz, XXL</option>
                            <?php endif; ?>
                        </select>
                    </div>
                    ...
                    <div class="buttons-section">
                        <a class="pass-btn" id="ty-btn-skip">Nein danke</a>
                        <div class="buy-btn" id="ty-btn-add" data-product-id="<?php echo esc_attr( $upsell_product_id ); ?>">ZUR BESTELLUNG HINZUFUEGEN</div>
                    </div>
                </div>
            ...
            <div class="ty-grid-popup">
                <div class="ty-grid-header" style="cursor:pointer;flex-direction:column;position:relative;">
                    ...
                    <div style="display:flex;align-items:center;justify-content:center;gap:10px;margin-bottom:8px;">
                        <div style="font-size:15px;font-weight:400;color:#000;">Sonderangebot endet in</div>
                    ...
                    <div style="font-size:20px;font-weight:700;color:#000;line-height:1.3;text-align:center;padding-top:10px;">Fuegen Sie ein beliebiges Produkt mit 50 % Rabatt hinzu</div>
                </div>
                <div class="ty-section-body open" id="ty-grid-body">
                <div class="ty-section-body-inner" style="padding:0;">
                <div class="ty-grid-trust">
                    ✔ Wir versenden alles im selben Paket
                </div>
                <div class="ty-grid">
                    ...
                    <div class="ty-grid-item">
                        ...
                        <div class="g-category">BOXERSHORTS</div>
                ...
                <div class="buttons-section">
                    <a class="pass-btn" id="ty-grid-close">Nein danke</a>
                    <div class="buy-btn" id="ty-grid-add-all">ZUR BESTELLUNG HINZUFUEGEN</div>
                </div>
        ...
        <div class="ty-section" id="ty-order-items-section">
            <div class="ty-section-header open" onclick="tyToggle(this)">
                <span id="ty-order-items-header">Bestellpositionen (<?php echo $order->get_item_count(); ?>)</span>
        ...
        <div class="ty-section">
            ...
            <div class="ty-section-body open">
                <div class="ty-section-body-inner">
                    ...
                    <div class="ty-row"><span class="ty-row-label">Adresse</span><span class="ty-row-value"><?php echo esc_html( $order->get_billing_address_1() . ' ' . $order->get_billing_address_2() ); ?></span></div>
                    <div class="ty-row"><span class="ty-row-label">Stadt</span><span class="ty-row-value"><?php echo esc_html( $order->get_billing_postcode() . ' ' . $order->get_billing_city() ); ?></span></div>
...
<?php else : ?>
    <div class="ty-container">
        <div class="ty-success"><h1>Bestellung</h1>
...
(function(){
    ...
    function tick() {
        if (rem <= 0) {
            ...
            // Auto-open "Bestellpositionen" and "Lieferadresse" sections
        ...
        fetch(ajaxUrl, { method: 'POST', body: rfd })
            ...
            .then(function(d) {
                if (d.success) {
                    ...
                    if (headerSpan) {
                        headerSpan.textContent = 'Bestellpositionen (' + d.data.item_count + ')';
                    }
        ...
        addBtn.addEventListener('click', function() {
            ...
            fetch(ajaxUrl, { method: 'POST', body: fd })
                ...
                .catch(function() {
                    ...
                    addBtn.textContent = 'ZUR BESTELLUNG HINZUFUEGEN';
                });
        });
    }
    ...
    if (overlay) {
        ...
        // "Nein danke" — close all, show summary without upsells
        document.getElementById('ty-grid-close').addEventListener('click', closeAll);

        // "ZUR BESTELLUNG HINZUFUEGEN" — add all selected items, then close
        ...
        if (gridAddAll) {
            gridAddAll.addEventListener('click', function() {
                ...
                gridAddAll.textContent = 'Wird hinzugefuegt...';
                ...
                fetch('<?php echo admin_url("admin-ajax.php"); ?>', { method: 'POST', body: rfd })
                    ...
                    .then(function(d2) {
                        if (d2.success) {
                            ...
                            var headerSpan = document.getElementById('ty-order-items-header');
                            if (headerSpan) {
                                headerSpan.textContent = 'Bestellpositionen (' + d2.data.item_count + ')';
                            }
                        }
                    });
            } else {
