...
<!-- suport section -->
	...
	<?php
	// ===== Klaviyo newsletter =====
	// TODO(DE): replace with the German footer list ID (Klaviyo -> Lists
	// & Segments -> Settings -> List ID). Left as placeholder so signups are
	// NOT sent to another market's list until the DE list is set.
	$nf_klaviyo_list = 'YyjfBd';
	...
	if ( ! $nf_brand ) {
		$nf_brand = 'NORIKS ist eine europäische Marke für Produkte des täglichen Gebrauchs. Wir konzentrieren uns auf Qualität, Komfort und faire Preise — mehr als 120.000 Kunden vertrauen uns bereits.';
	}
	$nf_bg      = get_field( 'footer_bg_image', 'option' ); // background photo behind the brand band
	$nf_legal   = get_field( 'footer_legal_text', 'option' );
	if ( ! $nf_legal ) {
		$nf_legal = 'NORIKS BRAND. Alle Rechte vorbehalten. Preise inkl. MwSt. Bilder dienen nur zur Veranschaulichung.';
	}
	?>
		...
		<section class="nf-signup">
			<div class="nf-wrap nf-signup-grid">
				<div class="nf-signup-copy">
					<h2 class="nf-signup-title">Werde Teil der NORIKS Familie</h2>
					<p class="nf-signup-sub">Exklusive Angebote und Neuheiten — erfahre es als Erster.</p>
				</div>
				<div class="nf-signup-form-col">
					<form class="nf-signup-form" data-klaviyo-list="<?php echo esc_attr( $nf_klaviyo_list ); ?>" novalidate>
						<label class="nf-field-label" for="nf-email">Deine E-Mail *</label>
						<div class="nf-field-row">
							<input type="email" id="nf-email" name="email" class="nf-input" placeholder="Deine E-Mail-Adresse" required autocomplete="email">
							<button type="submit" class="nf-btn nf-btn-light">Anmelden</button>
						</div>
						<p class="nf-signup-note">
							Mit der Anmeldung stimmst du dem Erhalt von E-Mails zu. Du kannst dich jederzeit abmelden.
						...
						<div class="nf-link-col">
							...
							if ( $nf_col2_l ) :
								foreach ( $nf_col2_l as $item ) :
									if ( false !== strpos( (string) $item['link'], '/anleitungen' ) ) { $nf_has_upute = true; }
									?>
								<a href="<?php echo esc_url( $item['link'] ); ?>"><?php echo esc_html( $item['text'] ); ?></a>
							<?php endforeach; endif; ?>
							<?php if ( ! $nf_has_upute ) : ?>
								<a href="/de/anleitungen/">Gebrauchsanleitungen</a>
							<?php endif; ?>
						</div>
					<?php endif; ?>
		...
		<div class="nf-wrap nf-meta">
			<div class="nf-meta-left">
				<span class="nf-region">
					<img loading="lazy" decoding="async" src="https://static.devit.software/countries/flags/rectangle/de.svg" alt="" class="nf-flag">
					Deutschland (DE)
				</span>
				<span class="nf-copy">© <?php echo date( 'Y' ); ?> NORIKS. Alle Rechte vorbehalten.</span>
			</div>
	...
	(function () {
		...
		form.addEventListener('submit', function (e) {
			...
			if (!email || email.indexOf('@') === -1) {
				msg.textContent = 'Bitte gib eine gültige E-Mail-Adresse ein.'; msg.className = 'nf-signup-msg is-error'; return;
			}
			if (!list || list === 'REPLACE_LIST_ID') {
				msg.textContent = 'Newsletter ist noch nicht verbunden (Klaviyo List ID fehlt).'; msg.className = 'nf-signup-msg is-error'; return;
			}
			msg.textContent = 'Anmeldung…'; msg.className = 'nf-signup-msg';
			...
			fetch('https://manage.kmail-lists.com/ajax/subscriptions/subscribe', {
				...
				if (d && d.success) {
					msg.textContent = 'Danke! Bitte bestätige deine Anmeldung per E-Mail.'; msg.className = 'nf-signup-msg is-ok';
					form.reset();
				} else {
					msg.textContent = (d && d.errors && d.errors[0]) ? d.errors[0] : 'Etwas ist schiefgelaufen, bitte versuche es erneut.';
					msg.className = 'nf-signup-msg is-error';
				}
			}).catch(function () {
				msg.textContent = 'Etwas ist schiefgelaufen, bitte versuche es erneut.'; msg.className = 'nf-signup-msg is-error';
			});
		});
	})();
	</script>
