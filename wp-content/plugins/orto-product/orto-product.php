...
function gck_get_bundle_offers( $product_id = null ) : array {
    ...
    foreach ( $rows as $row ) {
        ...
        $saving_text   = "Gesamtersparnis " . number_format($saving_amount, 2, '.', '') . "€";
...
function gck_split_attrs_color_size( array $custom_attrs ) : array {
    ...
    foreach ( $custom_attrs as $key => $attr ) {
        ...
        $is_color = ( strpos($hay, 'seite') !== false || strpos($hay, 'boja') !== false || strpos($hay, 'barva') !== false || strpos($hay, 'farb') !== false || strpos($hay, 'culoare') !== false || strpos($hay, 'color') !== false || strpos($hay, 'colour') !== false || strpos($hay, 'colore') !== false || strpos($hay, 'kolor') !== false || strpos($hay, 'szín') !== false || strpos($hay, 'szin') !== false || strpos($hay, 'ρώμ') !== false || strpos($hay, 'цвят') !== false );
...
function gck_de_majice_phrase( int $n, bool $free = false, string $type = 'majica' ) : string {
    if ( $type === 'steznica' ) {
        $noun = ( $n === 1 ) ? 'Kniebandage' : 'Kniebandagen';
        return $free ? ( 'gratis ' . $noun ) : $noun;
    }
    if ( $type === 'bokserica' ) {
        $noun = 'Boxershorts';
        return $free ? ( 'gratis ' . $noun ) : $noun;
...
function gck_render_bundle_selector() {
    ...
    if (
        ...
        || ( stripos( (string) $product->get_slug(), 'socke' ) !== false )
        || ( stripos( (string) $product->get_name(), 'Socke' ) !== false )
    ) {
    ...
    //  - orto-ortopas                 : single "Größe" attribute, no colour (size selector only).
    ...
    if ( has_term( array( 'orto-cloath' ), 'product_cat', $product_id ) ) {
        $gck_offer_subs = array( 'Set fürs Badezimmer', 'Paket fürs ganze Haus', 'Paket für Familie und Freunde' );
    } else {
        $gck_offer_subs = array();
    }
    $gck_offer_badges = has_term( array( 'orto-cloath', 'orto-cloud' ), 'product_cat', $product_id )
        ? array( 1 => 'BESTSELLER', 2 => 'BESTER PREIS' )
        : array();
  ...
  margin-right:6px;
    ...
    <div class="gck-benefits-box">
        ...
        <ul class="gck-benefits-list">
            <?php if ( !has_term( array( 'orto-bokserice', 'orto-bokserice2', 'starter-paketi' ), 'product_cat', $product_id ) ) : ?>
                <li><span class="gck-check">✔</span> <strong>Perfekte Passform</strong></li>
            <?php endif; ?>

            <li><strong>✔ Premium-Materialien und präziser Schnitt</strong></li>
            <li><strong>✔ Komfort ohne Kompromisse</strong></li>

            <?php if ( has_term( array( 'orto-starter' ), 'product_cat', $product_id ) ) : ?>
                <li style="color: #c00;"><strong>✔ Starterpaket nur einmal pro Person verfügbar</strong></li>
                <li style="color: #c00;"><strong>✔ Limitiert auf 1.000 Pakete</strong></li>
            <?php endif; ?>
        </ul>
        <?php endif; ?>
    ...
    <div class="gck-top-banner-wrap">
        <?php if ( has_term( array( 'orto-starter' ), 'product_cat', $product_id ) ) : ?>
           ...
           <span>Wählen Sie Ihr exklusives<br/> Starterpaket</span>
        </div>
    ...
    <?php if ( has_term( array( 'orto-starter' ), 'product_cat', $product_id ) ) : ?>
        <div class="dev-banner" data-total="1000" data-sold="354">
            <div class="dev-banner__text">
                ...
                <span>Begrenzte Menge <b class="sold">0</b> / <b class="total">0</b> (Noch <b class="remain">0</b> Stück)</span>
            </div>
    ...
    <?php if ( $show_countdown ) : ?>
        ...
        <div class="gck-countdown" id="gck-countdown"
            ...
            <div class="gck-countdown__body">
                Aktion gilt nur heute — noch <span class="gck-countdown__timer" aria-live="polite">--:--</span>.<?php
                if ( $countdown_stock > 0 ) :
                    ?> Nur noch <strong><?php echo esc_html( $countdown_stock ); ?> Stück</strong> übrig.<?php
                endif; ?> <strong>Beeilen Sie sich, bevor es ausverkauft ist!</strong>
            </div>
        </div>
        <script>
          (function(){
    ...
    <style>
      ...
      /* lokalizirane barve DE */
      #bundle-selector .swatch-circle.color-blau { background: #b7c4e6 !important; }
      #bundle-selector .swatch-circle.color-rosa { background: #e6a8b8 !important; }
      #bundle-selector .swatch-circle.color-grau { background: #c9c2ba !important; }
      #bundle-selector .swatch-circle.color-grun { background: #b3bf9a !important; }
      #bundle-selector .swatch-circle.color-lila { background: #b48fc4 !important; }
      #bundle-selector .swatch-circle.color-marineblau { background: #2b3a63 !important; }
    ...
    <style>
      ...
      /* lokalizirane barve DE */
      #bundle-selector .swatch-circle.color-schwarz { background: #17161a !important; }
      #bundle-selector .swatch-circle.color-dunkelgrau { background: #4a4a4f !important; }
      #bundle-selector .swatch-circle.color-rosa { background: #e28ba4 !important; }
        ...
        ?>
            <label style="position: relative; <?php if ( ( ( $loop_index == 1 || $loop_index == 3 ) && ! $show_group_titles ) || ( ! empty( $gck_offer_badges ) && isset( $gck_offer_badges[ $loop_index ] ) ) ) : ?> margin-top: 25px;  <?php endif; ?>"
                ...
                <?php if ( ! $show_group_titles ) : ?>
                    <?php if ( $loop_index == 1 ) : ?>
                        <div class="gck-popular-badge">Am beliebtesten 🔥</div>
                    <?php endif; ?>

                    <?php if ( $loop_index == 3 ) : ?>
                        <div class="gck-popular-badge-2">Bester Preis 🔥</div>
                    <?php endif; ?>
                <?php endif; ?>
    ...
    && !has_term( array( 'starter-paketi' ), 'product_cat', $product_id ) 
                ...
                <?php if ( $show_price_highlights ) : ?>
                    ...
                    <span class="gck-per-chip">
                        ...
                        <?php endif; ?>
                        <span class="gck-per-new"><?php echo number_format( $gck_per_new, 2 ); ?>€ / St.</span>
                    </span>
                ...
                <?php else : ?>
                    — <span class="bundle-option-title"><?php echo number_format( $gck_per_new, 2 ); ?>€ / St.</span>
                <?php endif; ?>
                ...
                <div class="bundle-total-line">
                    <span style="font-weight:normal;">Gesamt:</span>
...
<div class="bundle-total-line">
    ...
    <span class="line-total"><?php echo number_format( (float) $data['total'], 2 ); ?>€</span>
                ...
                <div class="bundle-pairs <?php echo $is_default ? '' : 'hidden'; ?>"
                    ...
                    // attributes — "Farbe T-Shirt"/"Größe T-Shirt", "Farbe Boxershorts"/
                    // "Größe Boxershorts", "Farbe Socken"/"Größe Socken" — grouped by the
                    // garment word via the same helper SHBOX uses ($attr_groups). Form
                    // field names stay stable (majica_color …) so cart/label capture and
                    // the socks-width CSS are unaffected. Falls back to hardcoded options
                    // if a garment group can't be resolved, so it never breaks. Each item
                    // gets its own pair index so all 6 selections reach the cart.
                    $gck_sections = array(
                        array(
                            'label'   => 'Wähle 4 T-Shirts',
                            'gratis'  => false, 'count' => 4, 'gg' => 0, 'garment' => 'majica',
                            'ckey'    => 'majica_color', 'skey' => 'majica_size',
                            'colors'  => array( 'Schwarz', 'Weiß', 'Grau', 'Beige', 'Dunkelblau', 'Braun', 'Grün' ),
                            'sizes'   => array( 'S', 'M', 'L', 'XL', '2XL', '3XL', '4XL' ),
                        ),
                        array(
                            'label'   => 'Wähle 1 Boxershorts gratis',
                            'gratis'  => true, 'count' => 1, 'gg' => 1, 'garment' => 'bokserica',
                            'ckey'    => 'bokserica_color', 'skey' => 'bokserica_size',
                            'colors'  => array( 'Schwarz', 'Blau', 'Grau', 'Grün', 'Rot' ),
                            'sizes'   => array( 'S', 'M', 'L', 'XL', '2XL', '3XL', '4XL' ),
                        ),
                        array(
                            'label'   => 'Wähle die Sockenfarbe (5 Paar)',
                            'gratis'  => true, 'count' => 1, 'gg' => 2, 'garment' => 'carapa',
                            'ckey'    => 'carapa_color', 'skey' => 'carapa_size',
                            'colors'  => array( 'Schwarz', 'Weiß' ),
                    ...
                    if ( $gck_split_garments ) {
                        $gck_passes = array(
                            array( 'group' => 0, 'label' => 'Wähle ' . (int) $pairs . ' ' . gck_de_majice_phrase( $pairs, false, 'majica' ), 'gratis' => false ),
                            array( 'group' => 1, 'label' => 'Wähle noch ' . (int) $pairs . ' ' . gck_de_majice_phrase( $pairs, true, 'bokserica' ), 'gratis' => true ),
                        );
                    } else {
                    ...
                    <?php for ( $i = 1; $i <= $pairs; $i++ ) : ?>
                        <?php if ( $gck_pass['group'] === null && $gck_show_sections && $gck_paid > 0 && $i === 1 ) : ?>
                            <div class="gck-pair-label">Wähle <?php echo (int) $gck_paid; ?> <?php echo esc_html( gck_de_majice_phrase( $gck_paid, false, $gck_garment ) ); ?></div>
                        <?php endif; ?>
                        <?php if ( $gck_pass['group'] === null && $gck_show_sections && $gck_free > 0 && $i === ( $gck_paid + 1 ) ) : ?>
                            <div class="gck-pair-label is-gratis">Wähle noch <?php echo (int) $gck_free; ?> <?php echo esc_html( gck_de_majice_phrase( $gck_free, true, $gck_garment ) ); ?></div>
                        <?php endif; ?>
                        <div class="bundle-pair">
                    ...
                    <?php if ( ! $gck_no_attrs ) : ?>
                    <small style="display: block; line-height: 1;"><?php esc_html_e( 'Wir bieten 30 Tage Geld-zurück-Garantie oder Umtausch – sorgenfreies Einkaufen!
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    form.addEventListener('submit', function (e) {
        ...
        if (!valid) {
            e.preventDefault();
            alert('Bitte wählen Sie Farbe und Größe für jedes Set.');
        }
    });
 ...
 * shows which garment was picked, e.g. "T-Shirt: Schwarz - L", "Boxershorts: Blau - M",
 * "Socken: Schwarz - 39-43".
 */
function gck_build_shgifts_lines( array $pairs_data ) : array {
    ...
    foreach ( $pairs_data as $attrs ) {
        ...
        foreach ( array_keys( $attrs ) as $k ) {
            ...
            if ( strpos( $kl, 'majic' ) !== false )    { $label = 'T-Shirt';     break; }
            if ( strpos( $kl, 'bokseric' ) !== false ) { $label = 'Boxershorts'; break; }
            if ( strpos( $kl, 'carap' ) !== false )    { $label = 'Socken';      break; }
        }
...
function gck_shgifts_free_gifts_notice() {
    ...
    $gifts = array(
        array( 'label' => '1x Boxershorts', 'img' => $boxer_img, 'price' => '19,99 €' ),
    );
    for ( $i = 0; $i < 5; $i++ ) {
        $gifts[] = array( 'label' => '1x Paar Socken', 'img' => $socks_img, 'price' => '12,99 €' );
    }
    ?>
    <style>
      .gck-fg{
    ...
    <div class="gck-fg">
        <div class="gck-fg__head">6 GRATIS-Artikel bei jedem Kauf</div>
